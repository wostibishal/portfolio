...
from backend.app.models.user_model import User as usermodule
...
def signup(user_data: UserCreate, db: Session = Depends(get_session)):
    ...
    statement = select(usermodule).where(usermodule.email == user_data.email)
...
def login(
    ...
    statement = select(usermodule).where(usermodule.email == form_data.username)
...
def read_user(user_id: str, db: Session = Depends(get_session)):
    user = db.get(usermodule, user_id)
    if not user:
...
def read_users(
    ...
    users = db.exec(select(usermodule).offset(skip).limit(limit)).all()
    return [UserRead.from_orm(user) for user in users]      

@router.get("/me/", response_model=UserRead)
async def read_current_user(
    current_user: usermodule = Depends(get_current_active_user),
):
